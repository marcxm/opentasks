...
function handleTimezoneForDates(taskData) {
  if (taskData.dtstart) {
    ...
    if (!dtstartStr.endsWith('Z') && !dtstartStr.includes('+') && !dtstartStr.includes('-', 10)) {
      // Parse the local time and treat it as UTC to preserve the local time meaning
      ...
      // Create a UTC date with the local time values to preserve the local time
      const utcDate = new Date(Date.UTC(year, month, day, hours, minutes, seconds));
      taskData.dtstart = utcDate.toISOString();
  ...
  if (taskData.due) {
    ...
    if (!dueStr.endsWith('Z') && !dueStr.includes('+') && !dueStr.includes('-', 10)) {
      ...
      // Create a UTC date with the local time values to preserve the local time
      const utcDate = new Date(Date.UTC(year, month, day, hours, minutes, seconds));
      taskData.due = utcDate.toISOString();
...
router.get('/', async (req, res) => {
  try {
    ...
    // Strip 'Z' from datetime fields to indicate floating time (no timezone conversion needed)
    tasks = tasks.map(task => ({
      ...task,
      list_id: task.collection,
      // Convert UTC times to floating times by removing 'Z'
      dtstart: task.dtstart ? task.dtstart.replace(/\.000Z$/, '') : null,
      due: task.due ? task.due.replace(/\.000Z$/, '') : null,
      completed: task.completed ? task.completed.replace(/\.000Z$/, '') : null,
      created: task.created ? task.created.replace(/\.000Z$/, '') : null,
      last_modified: task.last_modified ? task.last_modified.replace(/\.000Z$/, '') : null
    }));
